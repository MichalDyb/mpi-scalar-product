...
int main(int argc, char ** argv)
{
    ...
    MPI_Graph_create(MPI_COMM_WORLD, nnodes, index, edges,reorder, &ring_hyper_d3); // Utworzenie struktury grafu, według powyższej definicji sturktury
    ...
    if(rank == 0) // Instrukcje wykonywane dla głównego procesu programu (proces root)
    {
        ...
        MPI_Scatter(vt_2, vt_part_size, MPI_DOUBLE, vt_2_part, vt_part_size, MPI_DOUBLE, 0, ring_hyper_d3); // Wysłanie podzielonego wektora 1 na kawałki do procesów w grupie
        ...
        MPI_Barrier(ring_hyper_d3); // Bariera synchronizacyjna
        MPI_Gather(&score, 1, MPI_DOUBLE, scores, 1, MPI_DOUBLE, 0, ring_hyper_d3); // Zebranie wyników z wszystkich procesów grupie
    ...
    {
        MPI_Barrier(ring_hyper_d3); // Bariera synchronizacyjna
        MPI_Bcast(&vt_real_size, 1, MPI_INT, 0, ring_hyper_d3); // Odebranie rozmiaru wektora
        MPI_Bcast(&vt_part_size, 1, MPI_INT, 0, ring_hyper_d3); // Odebranie rozmiaru części wektora
        vt_1 = malloc(vt_real_size * sizeof(double)); // Zaalokowanie pamięci na pierwszy wektor
        vt_2 = malloc(vt_real_size * sizeof(double)); // Zaalokowanie pamięci na drugi wektor
        vt_1_part = malloc(vt_part_size * sizeof(double)); // Zaalokowanie pamięci na pierwszy wektor częsciowy
        vt_2_part = malloc(vt_part_size * sizeof(double)); // Zaalokowanie pamięci na drugi wektor częściowy

        MPI_Scatter(vt_1, vt_part_size, MPI_DOUBLE, vt_1_part, vt_part_size, MPI_DOUBLE, 0, ring_hyper_d3); // Odebranie podzielonego wektora 1 na kawałki
        MPI_Scatter(vt_2, vt_part_size, MPI_DOUBLE, vt_2_part, vt_part_size, MPI_DOUBLE, 0, ring_hyper_d3); // Odebranie podzielonego wektora 1 na kawałki
        for (int i = 0; i < vt_part_size; i++) // Obliczenie iloczynu częsciowych wektorów
        {
        ...
        fprintf(stderr, "Iloczyn skalarny procesora %d wynosi: %f\n", rank, score); // Wyświetlenie informacji o obliczonym częściowym iloczynie skalarnym
        MPI_Barrier(ring_hyper_d3); // Bariera synchronizacyjna
        MPI_Gather(&score, 1, MPI_DOUBLE, NULL, 1, MPI_DOUBLE, 0, ring_hyper_d3); // Wysłanie wyników do procesu zbierającego wyniki
